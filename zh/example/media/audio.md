    ...
    try:
        ...
        for i in range(0, int(RATE / CHUNK * duration)):
            ...
            if exit_check():
                break
        # 将列表中的数据保存到WAV文件
        wf = wave.open(filename, 'wb')  # 创建WAV文件
        wf.set_channels(CHANNELS)        # 设置WAV声道数
        wf.set_sampwidth(p.get_sample_size(FORMAT))  # 设置WAV采样精度
        wf.set_framerate(RATE)           # 设置WAV采样率
...
def play_audio(filename):
    try:
        ...
        CHUNK = int(wf.get_framerate() / 25)  # 设置音频块大小

        p = PyAudio()
        p.initialize(CHUNK)  # 初始化PyAudio对象
        MediaManager.init()   # 初始化vb buffer

        # 创建音频输出流，音频参数从WAV文件中获取
        ...
        while data:
            stream.write(data)  # 将帧数据写入音频输出流
            data = wf.read_frames(CHUNK)  # 继续从WAV文件中读取数据
            if exit_check():
                break
    except BaseException as e:
        print(f"异常：{e}")
...
def loop_audio(duration):
    ...
    try:
        ...
        print("输入音量：", input_stream.volume())
