...
**KPU（Knowledge Processing Unit）** 正是嘉楠科技专为应对边缘AI计算挑战而设计的硬件加速引擎。它本质上是一个高度优化的**深度学习协处理器/加速器**，其核心功能是**高效执行神经网络模型中的密集计算任务**（特别是卷积、矩阵乘法、激活函数等操作）。
...
它通常作为SoC（System on Chip）中的一个独立IP核存在，与CPU、其他加速器（如音频DSP）、内存、外设等协同工作。CPU负责系统管理、任务调度和应用逻辑，而将计算密集型的AI模型推理任务高效地交给KPU执行。下图展示了KPU在典型边缘AI SoC中的位置。
...
这些开箱即用的 Demo 涵盖了单模型应用（如人脸检测）和多模型应用（如手掌关键点）两大类别，让用户无需从零开始搭建，即可零配置、一键式地体验主流 AI 功能，包括但不限于：
...
数据采集是指从摄像头、麦克风等设备收集原始数据的过程。数据采集的质量和数量直接影响后续步骤的效果，因此选择合适的采集设备和采集方法至关重要。对于在K230上部署的模型，使用K230采集图像数据有助于实现更好的模型部署效果。

🏷️ **数据标注**:

数据标注是为收集的数据添加标签的过程，以便于训练监督学习模型。这一步骤可能涉及到人工标注，也可能使用自动化工具来辅助完成。比如对于图像分类任务，要指定每张图像的所属类别；在目标检测任务中，需要标识待检测实例的位置和类别信息。准确的数据标注对于构建高效且可靠的模型具有重要意义。

🏷️ **模型训练**:

在模型训练阶段，使用已标注的数据集来训练深度学习模型。模型训练过程是对采集数据的数据分布实现拟合的过程，您需要选择合适的模型和强大的计算资源实现这一过程。训练的目标是找到一组能够准确地对新数据做出预测的模型参数。训练完成后，会得到一个经过验证性能良好的模型。现在常用的模型训练框架为pytorch和tensorflow等，您可以根据自己的需求选择合适的框架。

🏷️ **模型转换和验证**:

由于边缘设备在硬件和计算能力方面存在限制，无法直接部署在高性能计算平台上训练得到的模型。为了在特定芯片上实现高效的模型推理，通常需要借助不同的运行时（runtime）来调用加速单元进行计算，而不同的运行时所支持的模型格式也各不相同。

对于 K230 芯片，其使用 KPU（神经网络加速单元）进行模型推理加速，所需的模型格式为 KModel。KModel 是通过神经网络编译器 nncase 将 ONNX 或 TFLite 模型转换得到的。在模型转换过程中，nncase 会对模型结构进行优化并执行量化处理，以降低计算复杂度和存储需求，从而使模型更加适合在边缘设备上运行。

此外，模型转换完成后，还需对其进行验证，确保在精度和性能方面没有出现明显下降，以满足实际应用的要求。

🏷️ **模型部署**:

当模型被成功转换并验证后，就可以将其部署到K230上。用户需要使用K230 SDK提供的API，将模型加载到K230的内存中，并配置相关参数以实现模型的推理。部署过程包含读入数据、数据预处理、模型推理、输出后处理、绘制结果等步骤。不同的模型可能需要不同的预处理和后处理方式，因此在部署过程中需要用户按**模型需求**完成这部分代码的编写。

🏷️ **模型调优**:

即使模型已经部署，工作仍未结束。根据模型在边缘设备的实际运行情况，可能需要进一步调整和优化模型，以提高其性能或降低资源消耗。这包括但不限于：调整部署阈值、调整模型转换参数、改进模型训练参数、改进采集的数据集、优化推理速度等。模型调优是一个持续的过程，旨在适应不断变化的需求和条件。

以上六个步骤构成了在K230芯片上完成AI推理的基本流程，每一步都需要仔细规划和执行，以确保最终应用开发的成功。
...
nncase软件栈包括**compiler**和**runtime**两部分。
...
🏷️ **Runtime**: 集成于用户App， 提供加载kmodel/设置输入数据/KPU执行/获取输出数据等功能。
...
⚠️ **注意**：在进行onnx推理和kmodel推理的时候，对于输入数据的处理要慎重，如果kmodel中包含了某种预处理，那么这个预处理操作需要对onnx的输入进行处理，对kmodel的处理则不需要，因为该预处理步骤在kmodel内部自动完成。kmodel内部封装的预处理支持：

- 交换RB通道，SwapRB参数相关；
- Transpose实现NCHW和NHWC互相转换，input_shape和input_layout参数相关；
- 数据标准化，mean和std参数相关；
- 数据反量化，input_type和input_range参数相关；

关于onnx模型和kmodel模型推理步骤的区别，请参考下图：

![inference_diff_onnx_kmodel](https://www.kendryte.com/api/post/attachment?id=612)

对于onnx模型，模型内部不包含任何预处理过程，因此在模型推理前需要对数据完成所需的所有预处理；对于kmodel，如果开启了`preprocess`，内部包含的预处理可以不用在模型外部完成，由kmodel配置自动完成，如果没有开启`preprocess`，和onnx模型相同，实现推理所需的所有预处理。

按照对应的步骤得到onnx模型和kmodel模型的输入数据，可以将输入数据保存成`bin`文件，用于推理时从本地加载。**生成过程必须按照模型需求处理，并不是所有模型都是相同的**。示例代码如下：
...
上述onnx模型和kmodel模型输入数据的处理存在一些差异：

- onnx模型存在数据标准化过程，kmodel模型在转换时设置了归一化参数，则不需要在外部处理，而是在kmodel推理时做标准化。
- onnx模型输入数据为float32类型，kmodel模型输入数据为uint8类型，这是由转换kmodel时输入类型和输入范围的反量化参数决定的。
- 因为SwapRB参数为False，所以在预处理时都需要将BGR通道转换为RGB通道，如果SwapRB参数为True，那么生成kmodel输入数据时这一步则不需要，因为该过程会被kmodel处理。

因此，关于预处理存在如下关系：

```shell
onnx模型外预处理= kmodel模型外预处理 + kmodel模型内预处理
```
...
AI模型推理一般耗时较长，从几毫秒至几百毫秒不等。如果按照`采集图像->格式转换->预处理->模型推理->后处理->原图绘制结果->屏幕显示` 这个处理流程，模型推理时间会导致图像显示的延迟较大，并且模型越大延时也越大。为了减少图像显示延迟，所以我们可采用双通道的方式进行处理，即一路通道用于显示，另一路通道用于模型推理，再使用推理后的结果绘制OSD,和显示通道通过硬件叠加处理一同显示。虽然绘制的效果(比如：检测框)会存在一定的延迟，但是画面的显示会更加流畅。双通道采图的流程如下：
...
获得输入的`runtime_tensor`并不一定符合模型要求的输入，比如摄像头获取的图像大小是`1280*720`，而模型要求的输入是`320*320`，这就需要进行预处理。预处理的方法有很多，比如：裁剪、缩放、归一化、填充等。具体使用哪些预处理方法，需要对标onnx模型的预处理，同时在进行kmodel转换的时候，有一部分预处理（比如标准化）已经被封装到kmodel中了，这一部分预处理不需要再部署的时候实现。

> ⚠️ **注意**：用户需要对预处理过程有一定的了解，比如等比例填充过程中，可以选择双边填充，即在上下左右都进行填充，也可以选择单边填充，也就是在上左或者下右单侧进行填充。不同的填充方法在后处理过程进行坐标复原时所做的计算是不一样的。

#### 6.2.2 ai2d模块介绍

在MicroPython方案中，常见预处理一般使用`nncase_runtime.ai2d`模块由硬件实现，`ai2d`模块支持的预处理方法有5种，包括：缩放(resize)、裁剪(crop)、填充(pad)、仿射变换(affine)和比特位右移(shift)。该模块的API文档见链接：[ai2d API文档](https://www.kendryte.com/k230_canmv/zh/main/zh/api/nncase/K230_CanMV_nncase_runtime_API%E6%89%8B%E5%86%8C.html#nncase-runtime-ai2d)。
